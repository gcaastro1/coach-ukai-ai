...
ai_buttons_regex = re.compile(r'<div className="flex items-center gap-2">\s*<button[\s\S]*?</button>\s*</div>', re.MULTILINE)
code = ai_buttons_regex.sub('', code, count=1)

# 2. Remove AI reasoning UI
ai_reasoning_ui_regex = re.compile(r'{\(aiReasoning \|\| aiAnalysis\) && \([\s\S]*?}\)', re.MULTILINE)
code = ai_reasoning_ui_regex.sub('', code, count=1)

# 3. Inject Guide Tab content before the Info Tab
guide_content = """{activeTab === 'guide' && (
...
print("Python repair 2 completed")
