...
function repair() {
  ...
  const imports = `import guidesData from '../data/guides.json';
...
`;
  ...
  const stateRegex = /const \[activeTab, setActiveTab\] = useState.*?;([\s\S]*?)const { isPlayerSaved, savePlayer, removePlayer } = useAccount\(\);/m;
  const newState = `const [activeTab, setActiveTab] = useState<'guide' | 'info' | 'skills' | 'resonance' | 'bonus'>('guide');
    ...
    for (let line of lines) {
      ...
      if (memoryMatch) {
        ...
        if (memory) {
          ...
          const colorClass = rarityColors[memory.rarity] || 'border-white/10';
  ...
  // 3. Remove handlers
  const handlersRegex = /const handleAnalyzeBuild = async \(\) => \{[\s\S]*?const handleSuggestPotentials = async \(\) => \{[\s\S]*?setIsSuggesting\(false\);\n    \}\n  \};/m;
  code = code.replace(handlersRegex, () => '');

  // 4. Update modal container and header
  code = code.replace(/<div className="fixed top-1\/2 left-1\/2 -translate-x-1\/2 -translate-y-1\/2 w-full max-w-md bg-\[#0f0f0f\] border border-gray-800 shadow-2xl z-50 rounded-2xl overflow-hidden flex flex-col max-h-\[90vh\]">[\s\S]*?<div className="relative h-32 shrink-0 bg-neutral-900 border-b border-gray-800 flex items-center p-5 overflow-hidden">/m, () => \`<div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl bg-[#0a0a0a] border border-white/10 shadow-[0_0_100px_rgba(0,0,0,1)] z-50 rounded-3xl overflow-hidden flex flex-col max-h-[95vh] ring-1 ring-white/5">
  ...
  // 5. Update tabs
  const tabsRegex = /{\/\* Tabs \*\/}      \n        <div className="flex border-b border-gray-800 bg-\[#121212\] shrink-0">[\s\S]*?<div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin scrollbar-thumb-white\/10">/m;
  const newTabs = `{/* Tabs */}
  ...
  const finalCode = code.replace(tabsRegex, () => newTabs);
  fs.writeFileSync('src/components/PlayerDetailsModal.tsx', finalCode);
